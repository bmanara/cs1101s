...
function apply_binary(operator, op1, op2) {
    return operator === "+"
           ...
           : operator === "/" 
           ? op1 / op2
           : operator === "%" // Question 1 (Change to Hypotenuse Operator) op1 % op2
           ? math_sqrt(math_pow(op1, 2) + math_pow(op2, 2)) // Works for (a) and (b)
           : operator === "<" 
           ? op1 < op2
           : operator === ">" 
           ? op1 > op2
...
function logical_composition_first_component(component) {
    return list_ref(component, 2);
}

function logical_composition_second_component(component) {
    return list_ref(component, 3);
}
