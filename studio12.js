...
function evaluate(program) { 
    ...
    while (! is_null(C)) {
        ...
        if (is_literal(command)) {
            ...
            display("-----------START------------");
            display(function_expression(command));
            display("------------END-------------");
...
function sort_sequence(xs) { // Question 1
    function find_function_declaration(xs) {
        return accumulate((x, y) => 
                              head(x) === "function_declaration" 
                              ? x 
                              : y, 
                           head(xs), tail(xs));
    }
    ...
    if (is_null(xs)) {
        ...
        const command = find_function_declaration(xs);
        return pair(command, sort_sequence(remove(command, xs)));
    }
}
